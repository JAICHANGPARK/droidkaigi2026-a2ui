...
3. **`A2uiDataPath`의 KDoc이 RFC 6901에서 벗어나는 지점을 명시합니다.** 빈 문자열과 `"/"`는 둘 다 루트, 끝 슬래시는 *정확히 하나만* 제거 — 그 이유까지 주석에 있습니다("to maintain parsing parity with other A2UI renderers"). 스펙의 모호함이 코드 주석으로 해결된 사례라, 직접 렌더러를 쓸 때 적합성 테스트로 쓸 수 있습니다.
...
- `collectMessages()`의 `finally`에서 모든 서피스를 `dispose()` — 구조적 동시성으로 정리됩니다. 수동 해제 API가 없습니다.
